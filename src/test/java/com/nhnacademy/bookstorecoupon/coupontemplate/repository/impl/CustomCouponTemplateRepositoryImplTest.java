...
class CustomCouponTemplateRepositoryImplTest {
	...
	// 	)).thenReturn(jpaQuery);
	//
	// 	// Ensure method chaining returns the same object
	// 	when(jpaQuery.from(qCouponTemplate)).thenReturn(jpaQuery);
	// 	when(jpaQuery.join(qCouponTemplate.couponPolicy, qCouponPolicy)).thenReturn(jpaQuery);
	// 	when(jpaQuery.where(
	// 		qCouponPolicy.isUsed.isTrue()
	// 			.and(qCouponPolicy.type.in(types))
	// 			.and(qCouponTemplate.quantity.gt(0))
	// 			.and(qCouponTemplate.expiredDate.after(LocalDateTime.now()))
	// 	)).thenReturn(jpaQuery);
	// 	when(jpaQuery.orderBy(qCouponTemplate.id.desc())).thenReturn(jpaQuery);
	// 	when(jpaQuery.offset(anyLong())).thenReturn(jpaQuery);
	// 	when(jpaQuery.limit(anyLong())).thenReturn(jpaQuery);
	// 	when(jpaQuery.fetch()).thenReturn(tuples);
	//
	// 	// Mocking JPAQuery for counting
	// 	when(queryFactory.select(qCouponTemplate.id.count())).thenReturn(countQuery);
	// 	when(countQuery.from(qCouponTemplate)).thenReturn(countQuery);
	// 	when(countQuery.join(qCouponTemplate.couponPolicy, qCouponPolicy)).thenReturn(countQuery);
	...
	//
	// 	// When
	// 	Page<CouponTemplateResponseDTO> result = repository.findAllTemplatesByUserPaging(pageable, bookIdMap, categoryIdMap);
	//
	// 	// Then
	// 	assertEquals(2, result.getTotalElements());
	// 	assertEquals(2, result.getContent().size());
	...
	// 	verify(queryFactory).select(qCouponTemplate.id.count());
	// 	verify(countQuery).from(qCouponTemplate);
	// 	verify(countQuery).join(qCouponTemplate.couponPolicy, qCouponPolicy);
	// 	verify(countQuery).where(
	// 		qCouponPolicy.isUsed.isTrue()
	// 			.and(qCouponPolicy.type.in(types))
	// 			.and(qCouponTemplate.quantity.gt(0))
	// 			.and(qCouponTemplate.expiredDate.after(LocalDateTime.now()))
